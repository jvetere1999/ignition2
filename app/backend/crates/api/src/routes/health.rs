...
async fn debug_env(State(state): State<Arc<AppState>>) -> Json<DebugEnvResponse> {
    ...
    Json(DebugEnvResponse {
        oauth_google_configured: google_config.map(|g| !g.client_id.is_empty() && !g.client_secret.is_empty()).unwrap_or(false),
        oauth_azure_configured: azure_config.map(|a| !a.client_id.is_empty() && a.tenant_id.is_some()).unwrap_or(false),
        google_client_id_present: google_config.map(|g| !g.client_id.is_empty()).unwrap_or(false),
        google_client_secret_present: google_config.map(|g| !g.client_secret.is_empty()).unwrap_or(false),
        azure_client_id_present: azure_config.map(|a| !a.client_id.is_empty()).unwrap_or(false),
        azure_tenant_id_present: azure_config.and_then(|a| a.tenant_id.as_ref()).map(|t| !t.is_empty()).unwrap_or(false),
